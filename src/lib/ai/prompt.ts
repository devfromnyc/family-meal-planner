...
export function buildMealPlanPrompt(ctx: AiContext): string {
  const lines = [
    ...
    "Each meal object fields: title, mealType (breakfast|lunch|dinner), description, ingredients([{name,quantity,unit?}]), steps([string]), skillLevel (beginner|intermediate|hard), cookTimeMinutes (number), servings (number), whyItFits (string).",
